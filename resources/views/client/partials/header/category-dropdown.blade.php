                ...
                @if ($categories->count() > 0)
                    @foreach ($categories as $category)
                        <li class="megamenu-container">
                            <a class="sf {{ !empty($category->childrens) && $category->childrens->count() > 0 ? '-with-ul' : '' }}"
                                href="{{ route('list', ['category_slug' => $category->slug]) }}">{{ $category->name }}</a>
                            <div class="megamenu">
                                            ...
                                            <div class="row">
                                                <div class="col-md-6">
                                                    @foreach ($category->childrens as $children)
                                                        <a href="{{ route('list', ['category_slug' => $children->slug]) }}"
                                                            class="menu-title">{{ $children->name }}
                                                            ></a><!-- End .menu-title -->
                                                        {{-- <ul>
