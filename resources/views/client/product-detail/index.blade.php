                    ...
                    <div class="product-details-top">
                        <div class="row">
                                    ...
                                    <figure class="product-main-image">
                                        @if ($product->discount_percentage > 0)
                                            <span class="product-label label-sale"
                                                style="font-size: 15px; font-weight: bold">Giảm giá
                                                {{ $product->discount_percentage }}%</span>
                                        @endif
                                        <img style="" id="product-zoom" src="{{ $product->featured_image }}"
                                            class="lazyload" data-src="{{ $product->featured_image }}"
                            ...
                            <style>
                                .loading-overlay {
                                    position: fixed;
                                    /* Đặt overlay ở chế độ fixed */
                                    top: 0;
                                    /* Bắt đầu từ vị trí trên cùng */
                                    left: 0;
                                    /* Bắt đầu từ bên trái */
                                    width: 100%;
                                    /* Chiều rộng 100% */
                                    ...
                                    z-index: 9999;
                                    /* Đặt lên trên các thành phần khác */
                                    display: flex;
                                ...
                                <div class="product-details product-details-sidebar">
                                    ...
                                    <div class="ratings-container">
                                        ...
                                        <a class="ratings-text" href="#product-review-link" id="review-link">(
                                            {{ count($product->reviews) }} Đánh giá )</a>
                                    </div>
                                        ...
                                        <div class="product-cat">
                                            ...
                                            @if ($product->category->childrens->isNotEmpty())
                                                ...
                                                @foreach ($product->category->childrens as $subcategory)
                                                    <a
                                                        href="#">{{ $subcategory->name }}</a>{{ !$loop->last ? ', ' : '' }}
                                                @endforeach
                                            @endif
                                        </div><!-- End .product-cat -->
                ...
                <aside class="col-lg-3">
                    <div class="sidebar sidebar-product">
                        <div class="widget widget-products">
                            <h4 style="text-align: center;font-size: 20px" class="widget-title">Sản phẩm liên quan</h4>
                            <!-- End .widget-title -->

                            <div class="products">
                                @forelse($relatedProducts as $product)
                                    <div class="product product-sm"
                                        style="border: 0.1rem solid #dadada; border-radius: 10px; ">
                                        ...
                                        <div class="product-body">
                                            <h5 class="product-title"><a
                                                    href="{{ route('product', ['product_slug' => $product->slug]) }}">{{ Str::limit($product->name, 50) }}</a>
                                            </h5><!-- End .product-title -->
                                            <div class="product-price">
                                                @if ($product->discount_percentage > 0)
                                                    <span
                                                        class="original-price">{{ formatVND($product->base_price) }}</span>
                                                    <span class="final-price">{{ formatVND($product->sale_price) }}</span>
                                                @else
                                                    <span class="base-price">{{ formatVND($product->base_price) }}</span>
                                                @endif
                ...
                <ul class="nav nav-pills justify-content-center" role="tablist">
                    <li class="nav-item">
                        <a class="nav-link" id="product-info-link" data-toggle="tab" href="#product-info-tab" role="tab"
                            aria-controls="product-info-tab" aria-selected="true">Thông tin chi tiết</a>
                    </li>
                    <li class="nav-item">
